  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/80 backdrop-blur-xl overflow-y-auto">
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2.5 rounded-xl bg-slate-800/80 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors apple-press"
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {project.keyFeatures.map((feat, i) => (
                <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-950/50 border border-slate-800/70 text-xs text-slate-200">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {project.screenshots.map((sc, i) => (
                  <div key={i} className="rounded-2xl border border-slate-800 overflow-hidden bg-slate-950 p-4">
                    <div className="font-mono text-xs font-semibold text-slate-200 mb-1">{sc.caption}</div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t border-slate-800">
          <div className="text-xs font-mono text-slate-400">
            Client Type: <span className="text-slate-200 font-semibold">{project.clientType}</span>
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-md apple-press"
            >
