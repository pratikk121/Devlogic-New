...
export const EnterpriseProductPreview: React.FC<EnterpriseProductPreviewProps> = ({
  ...
  const fleetData = [
    ...
    { id: 'TRK-904', driver: 'Elena Rostova', location: 'Dallas, TX (US-75)', speed: '58 mph', status: 'ACTIVE', fuel: '92%', temp: '70°F', eta: '16:15 CST' },
    { id: 'TRK-882', driver: 'David Chen', location: 'Houston, TX (Terminal 4)', speed: '0 mph', status: 'IDLE', fuel: '45%', temp: '72°F', eta: 'At Depot' },
    { id: 'TRK-740', driver: 'Sarah Jenkins', location: 'San Antonio, TX (Depot B)', speed: '0 mph', status: 'MAINTENANCE', fuel: '18%', temp: '71°F', eta: 'In Service' },
    { id: 'TRK-915', driver: 'Carlos Gomez', location: 'Fort Worth, TX (I-20 West)', speed: '65 mph', status: 'ACTIVE', fuel: '76%', temp: '69°F', eta: '15:45 CST' }
  ];

  const filteredFleet = fleetData.filter((item) => {
    if (statusFilter !== 'ALL' && item.status !== statusFilter) return false;
    if (searchQuery && !item.id.toLowerCase().includes(searchQuery.toLowerCase()) && !item.driver.toLowerCase().includes(searchQuery.toLowerCase()) && !item.location.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div className="dark-product-surface rounded-2xl border border-slate-800 shadow-2xl overflow-hidden font-sans text-slate-100">
      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="bg-slate-950/80 border border-slate-800/80 rounded-xl p-4">
            ...
            <span className="text-[10px] text-emerald-400 font-mono flex items-center gap-1 mt-1">
              <ArrowUpRight className="w-3 h-3" /> Optimal Route Engine
            </span>
          </div>
        ...
        {activeTab === 'telemetry' && (
          <div className="bg-slate-950/90 rounded-xl border border-slate-800/90 overflow-hidden">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {filteredFleet.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-900/50 transition-colors">
                      <td className="px-4 py-3 font-bold text-cyan-400">{row.id}</td>
        ...
        {activeTab === 'drivers' && (
          <div className="bg-slate-950/90 rounded-xl border border-slate-800 p-5 space-y-4 font-mono text-xs">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <span className="font-bold text-white">DRIVER COMPLIANCE & SHIFT LOGS</span>
              <span className="text-emerald-400 text-[11px]">DOT HOS COMPLIANT</span>
            </div>

            <div className="space-y-3">
              {[
                { name: 'Marcus Vance', shift: '06:00 - 16:00 CST', driveTime: '6h 12m', restRemaining: '3h 48m', status: 'ON ROUTE' },
                { name: 'Elena Rostova', shift: '07:00 - 17:00 CST', driveTime: '5h 45m', restRemaining: '4h 15m', status: 'ON ROUTE' },
                { name: 'David Chen', shift: '08:00 - 18:00 CST', driveTime: '4h 20m', restRemaining: '5h 40m', status: 'DEPOT REST' }
              ].map((d, i) => (
                <div key={i} className="p-3.5 rounded-lg bg-slate-900/60 border border-slate-800/80 flex items-center justify-between">
                  <div>
                  ...
                  <div className="flex items-center gap-6 text-slate-300">
                    <div>
                      ...
                      <span className="font-bold text-cyan-400">{d.driveTime}</span>
                    </div>
                    <div>
                    ...
                    <span className="px-2 py-1 rounded bg-slate-950 text-slate-300 border border-slate-800 font-bold text-[10px]">
                      {d.status}
                    </span>
                  </div>
                </div>
        ...
        {activeTab === 'routes' && (
          <div className="bg-slate-950/90 rounded-xl border border-slate-800 p-5 space-y-4 font-mono text-xs">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <span className="text-cyan-400 text-[11px]">ALGORITHM: DIJKSTRA + TRAFFIC API</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-slate-900/60 border border-slate-800">
                <span className="text-cyan-400 font-bold block mb-1">Route #104: Austin → Dallas</span>
                <p className="text-slate-400 text-[11px] mb-3">Distance: 195 miles · Avg Speed: 62 mph · ETA Savings: -24 mins</p>
                <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-800">
                  <div className="bg-emerald-400 h-full w-[88%] rounded-full"></div>
                </div>
              </div>

              <div className="p-4 rounded-lg bg-slate-900/60 border border-slate-800">
                <span className="text-cyan-400 font-bold block mb-1">Route #208: Houston → San Antonio</span>
                <p className="text-slate-400 text-[11px] mb-3">Distance: 197 miles · Avg Speed: 59 mph · ETA Savings: -18 mins</p>
                <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-800">
                  <div className="bg-cyan-400 h-full w-[92%] rounded-full"></div>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'health' && (
          <div className="bg-slate-950/90 rounded-xl border border-slate-800 p-5 space-y-4 font-mono text-xs">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <span className="font-bold text-white">CLOUD RUN CONTAINER & INFRASTRUCTURE LOGS</span>
              <span className="text-emerald-400 text-[11px]">99.99% UPTIME GUARANTEE</span>
            </div>

            <div className="space-y-2">
              <div className="p-2.5 rounded bg-slate-900/80 border border-slate-800 flex items-center justify-between text-emerald-400">
                <span>[10:14:02] WebSocket Gateway: 1,280 active connections synchronized.</span>
                <span className="text-slate-500">2ms</span>
              </div>
              <div className="p-2.5 rounded bg-slate-900/80 border border-slate-800 flex items-center justify-between text-cyan-400">
                <span>[10:14:00] PostgreSQL Connection Pool: 12/50 connections active (HEALTHY).</span>
                <span className="text-slate-500">1ms</span>
              </div>
              <div className="p-2.5 rounded bg-slate-900/80 border border-slate-800 flex items-center justify-between text-slate-300">
